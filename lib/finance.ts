...
export function validateBackup(input: unknown): Backup {
  ...
  for (const item of input.budgets as unknown[]) { const limit = isRecord(item) ? item.limitChetrum : undefined; if (!isRecord(item) || typeof item.id !== 'string' || typeof item.month !== 'string' || !/^\d{4}-\d{2}$/.test(item.month) || typeof item.categoryId !== 'string' || typeof limit !== 'number' || !Number.isSafeInteger(limit) || limit <= 0 || !validIso(item.createdAt) || !validIso(item.updatedAt)) throw new Error('Invalid budgets data.') }
  for (const item of input.recurring as unknown[]) { const amount = isRecord(item) ? item.amountChetrum : undefined; const day = isRecord(item) ? item.dayOfMonth : undefined; if (!isRecord(item) || typeof item.id !== 'string' || typeof item.name !== 'string' || !['income', 'expense'].includes(item.type as string) || typeof amount !== 'number' || !Number.isSafeInteger(amount) || amount <= 0 || typeof item.categoryId !== 'string' || typeof item.paymentMethod !== 'string' || item.frequency !== 'monthly' || typeof day !== 'number' || !Number.isInteger(day) || day < 1 || day > 31 || !['essential', 'flexible'].includes(item.classification as string) || typeof item.isCommitment !== 'boolean' || typeof item.active !== 'boolean' || !validIso(item.createdAt) || !validIso(item.updatedAt) || (item.lastConfirmedMonth !== undefined && (typeof item.lastConfirmedMonth !== 'string' || !/^\d{4}-\d{2}$/.test(item.lastConfirmedMonth)))) throw new Error('Invalid recurring data.') }
