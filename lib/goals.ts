...
export function updateGoal(goal: Goal, input: Pick<Goal, 'name' | 'targetChetrum' | 'targetDate'>): Goal { const next = { ...goal }; delete next.targetDate; return { ...next, name: input.name.trim(), targetChetrum: input.targetChetrum, ...(input.targetDate ? { targetDate: input.targetDate } : {}), updatedAt: new Date().toISOString() } }
