...
// Display-only: turns an internal month key like "2026-09" into a human label
// like "Sep 2026". The underlying month key is never changed.
export function formatMonthLabel(month: string): string { const [year, value] = month.split('-').map(Number); if (!year || !value || value < 1 || value > 12) return month; return new Intl.DateTimeFormat('en', { month: 'short', year: 'numeric' }).format(new Date(year, value - 1, 1)) }
