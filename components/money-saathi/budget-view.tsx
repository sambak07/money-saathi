...
export function BudgetView({ transactions, onConfirmRecurring, onPlanningAudit, onPlanningChanged }: Props) {
  ...
  const overCount = rows.filter(row => row.overspent).length
  ...
  const recurringRef = useModalDialog<HTMLFormElement>({ open: showRecurring, onClose: () => { if (!recurringSaving) setShowRecurring(false) }, closable: !recurringSaving })
  ...
  return <>
    ...
    {showRecurring && <div className="modal-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget && !recurringSaving) setShowRecurring(false) }}><form className="add-sheet recurring-form" role="dialog" aria-modal="true" aria-label={editingRecurring ? 'Edit regular item' : 'Add regular item'} ref={recurringRef} onSubmit={submitRecurring}><div className="sheet-header"><div><p className="eyebrow">Regular money</p><h2>{editingRecurring ? 'Edit regular item' : 'Add regular item'}</h2></div><button type="button" className="round-button small" onClick={() => setShowRecurring(false)} aria-label="Close">×</button></div><label>Name<input value={name} onChange={event => setName(event.target.value)} required/></label><div className="form-grid"><label>Type<select value={recurringType} onChange={event => { const value = event.target.value as 'income' | 'expense'; setRecurringType(value); setRecurringCategory(value === 'income' ? 'salary' : 'food'); setIsCommitment(false) }}><option value="expense">Payment</option><option value="income">Income</option></select></label><label>Amount<input inputMode="decimal" value={recurringAmount} onChange={event => setRecurringAmount(event.target.value)} required/></label><label>Category<select value={recurringCategory} onChange={event => setRecurringCategory(event.target.value)}>{(recurringType === 'income' ? incomeCategories : expenseCategories).map(category => <option key={category.id} value={category.id}>{category.label}</option>)}</select></label><label>Payment method<select value={paymentMethod} onChange={event => setPaymentMethod(event.target.value)}>{PAYMENT_METHODS.map(method => <option key={method}>{method}</option>)}</select></label><label>Day of month<input type="number" min="1" max="31" step="1" value={dayOfMonth} onChange={event => setDayOfMonth(event.target.value)} required/></label>{recurringType === 'expense' && <label>Kind<select value={classification} onChange={event => setClassification(event.target.value as 'essential' | 'flexible')}><option value="essential">Essential</option><option value="flexible">Flexible</option></select></label>}</div>{recurringType === 'expense' && <label className="check-row"><input type="checkbox" checked={isCommitment} onChange={event => setIsCommitment(event.target.checked)}/> Treat as a fixed monthly payment</label>}{recurringError && <p className="form-error" role="alert">{recurringError}</p>}<button className="primary-button" type="submit" disabled={recurringSaving}>{recurringSaving ? 'Saving…' : editingRecurring ? 'Save changes' : 'Add regular item'}</button></form></div>}
  </>
}
